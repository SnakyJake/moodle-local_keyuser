 ...
 *
 * @copyright 2021 Jakob Heinemann, 1999 Martin Dougiamas  http://dougiamas.com
...
require_once($CFG->dirroot.'/local/keyuser/user/editadvanced_form.php');
require_once($CFG->dirroot.'/local/keyuser/user/editlib.php');
require_once($CFG->dirroot.'/local/keyuser/user/profile/lib.php');
...
$PAGE->set_url('/local/keyuser/user/editadvanced.php', array('course' => $course, 'id' => $id));
...
if ($id == -1) {
    ...
    $user->deleted = 0;
    $user->timezone = '99';
    require_capability('local/keyuser:usercreate', $systemcontext);
    admin_externalpage_setup('keyuser_addnewuser', '', array('id' => -1));
} else {
...
// Create form.
$userform = new keyuser_user_editadvanced_form(new moodle_url($PAGE->url, array('returnto' => $returnto)), array(
...
if ($userform->is_cancelled()) {
    ...
    if ($user->id == $USER->id) {
        ...
        redirect("$CFG->wwwroot/local/keyuser/admin/user.php");
    }
    // Never reached..
}
