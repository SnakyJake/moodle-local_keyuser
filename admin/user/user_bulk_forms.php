...
class user_bulk_action_form extends moodleform {
    ...
    public function get_actions(): array {
        ...
        $actions = [];
        /*
        if (has_capability('local/keyuser:userupdate', $syscontext)) {
            $actions['confirm'] = new action_link(
                new moodle_url('/local/keyuser/admin/user/user_bulk_confirm.php'),
                get_string('confirm'));
        }
        */
        /*
        if (has_capability('moodle/site:readallmessages', $syscontext) && !empty($CFG->messaging)) {
            $actions['message'] = new action_link(
                new moodle_url('/local/keyuser/admin/user/user_bulk_message.php'),
                get_string('messageselectadd'));
        }
        */
        /*
        if (has_capability('local/keyuser:userdelete', $syscontext)) {
            $actions['delete'] = new action_link(
                new moodle_url('/local/keyuser/admin/user/user_bulk_delete.php'),
                get_string('delete'));
        }
        */
        $actions['displayonpage'] = new action_link(
                new moodle_url('/local/keyuser/admin/user/user_bulk_display.php'),
                get_string('displayonpage'));

        if (has_capability('local/keyuser:userupdate', $syscontext)) {
            $actions['download'] = new action_link(
                new moodle_url('/local/keyuser/admin/user/user_bulk_download.php'),
        ...
        if (has_capability('local/keyuser:userupdate', $syscontext)) {
            $actions['forcepasswordchange'] = new action_link(
                new moodle_url('/local/keyuser/admin/user/user_bulk_forcepasswordchange.php'),
                get_string('forcepasswordchange'));
        }
        */
        if (has_capability('local/keyuser:cohortassign', $syscontext)) {
            $actions['addtocohort'] = new action_link(
                new moodle_url('/local/keyuser/admin/user/user_bulk_cohortadd.php'),
