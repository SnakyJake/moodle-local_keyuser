...
require_once($CFG->dirroot.'/local/keyuser/user/filters/lib.php');
require_once($CFG->dirroot.'/local/keyuser/admin/user/lib.php');
require_once($CFG->dirroot.'/local/keyuser/admin/user/user_bulk_forms.php');
...
$userbulkform = new user_bulk_form(null, keyuser_get_selection_data($ufiltering));

if ($data = $userbulkform->get_data()) {
    if (!empty($data->addall)) {
        keyuser_add_selection_all($ufiltering);
