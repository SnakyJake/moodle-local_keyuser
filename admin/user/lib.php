...
function keyuser_add_selection_all($ufiltering) {
...
function keyuser_get_selection_data($ufiltering) {
    global $SESSION, $DB, $CFG;

    // get the SQL filter
    $sqlwhere = "id<>:exguest AND deleted <> 1";
    $params = array('exguest'=>$CFG->siteguest);

    keyuser_user_append_where($sqlwhere, $params);

    $total  = $DB->count_records_select('user', $sqlwhere, $params);
