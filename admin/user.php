s<?php
    ...
    admin_externalpage_setup('keyuser_editusers');

    $sitecontext = context_system::instance();
    $site = get_site();

    if (!has_capability('local/keyuser:userupdate', $sitecontext) and !has_capability('local/keyuser:userdelete', $sitecontext)) {
    ...
    $returnurl = new moodle_url('/local/keyuser/admin/user.php', array('sort' => $sort, 'dir' => $dir, 'perpage' => $perpage, 'page'=>$page));

    // The $user variable is also used outside of these if statements.
    $user = null;
    if ($confirmuser and confirm_sesskey()) {
        require_capability('local/keyuser:userupdate', $sitecontext);
        $sql = "SELECT * FROM {user}";
        $wheresql = " WHERE id = :id AND mnethostid = :mnethostid";
        ...
        $sql = "SELECT * FROM {user} ";
        $wheresql = "WHERE id = :id AND deleted = 0 AND mnethostid = :mnethostid";
        ...
        require_capability('local/keyuser:userdelete', $sitecontext);
        $sql = "SELECT * FROM {user} ";
        $wheresql = "WHERE id = :id AND deleted = 0 AND mnethostid = :mnethostid";
        $params = ['id'=>$delete, 'mnethostid'=>$CFG->mnet_localhost_id];
        keyuser_user_append_where($wheresql,$params);
        $user = $DB->get_record_sql($sql . $wheresql, $params, IGNORE_MISSING);
        ...
        if (!has_capability('local/keyuser:userupdate', $sitecontext)) {
            print_error('nopermissions', 'error', '', 'modify the NMET access control list');
        }
        $sql = "SELECT * FROM {user} ";
        $wheresql = "WHERE id = :id AND deleted = 0 AND mnethostid = :mnethostid";
        $params = ['id'=>$acl, 'mnethostid'=>$CFG->mnet_localhost_id];
        keyuser_user_append_where($wheresql,$params);
        $user = $DB->get_record_sql($sql . $wheresql, $params, IGNORE_MISSING);
        ...
        $sql = "SELECT * FROM {user} ";
        $wheresql = "WHERE id = :id AND deleted = 0 AND mnethostid = :mnethostid";
        $params = ['id'=>$suspend, 'mnethostid'=>$CFG->mnet_localhost_id];
        keyuser_user_append_where($wheresql,$params);
        $user = $DB->get_record_sql($sql . $wheresql, $params, IGNORE_MISSING);
        ...
        $sql = "SELECT * FROM {user} ";
        $wheresql = "WHERE id = :id AND deleted = 0 AND mnethostid = :mnethostid";
        $params = ['id'=>$unsuspend, 'mnethostid'=>$CFG->mnet_localhost_id];
        keyuser_user_append_where($wheresql,$params);
        $user = $DB->get_record_sql($sql . $wheresql, $params, IGNORE_MISSING);
        ...
        $sql = "SELECT * FROM {user} ";
        $wheresql = "WHERE id = :id AND deleted = 0 AND mnethostid = :mnethostid";
        $params = ['id'=>$unlock, 'mnethostid'=>$CFG->mnet_localhost_id];
        keyuser_user_append_where($wheresql,$params);
    ...
    list($keyuser_extrasql, $keyuser_params) = $ufiltering->get_keyuser_sql_filter();
    $users = get_users_listing($sort, $dir, $page*$perpage, $perpage, '', '', '',
            $extrasql, $params, $context);
    $usercount = get_users(false, '', false, null, "", '', '', '', '', '*', $keyuser_extrasql, $keyuser_params);
    ...
    $baseurl = new moodle_url('/local/keyuser/admin/user.php', array('sort' => $sort, 'dir' => $dir, 'perpage' => $perpage));
    ...
    if (!$users) {
        ...
        foreach ($users as $user) {
            ...
            // delete button
            if (has_capability('local/keyuser:userdelete', $sitecontext)) {
            ...
            if (has_capability('local/keyuser:userupdate', $sitecontext)) {
                ...
                if (is_siteadmin($USER) or !is_siteadmin($user)) {
                    $url = new moodle_url('/local/keyuser/user/editadvanced.php', array('id'=>$user->id, 'course'=>$site->id));
    ...
    if (has_capability('local/keyuser:usercreate', $sitecontext)) {
        $url = new moodle_url('/local/keyuser/user/editadvanced.php', array('id' => -1));
