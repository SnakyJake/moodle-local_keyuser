...
//limit plugin to system context for now
$contextid = null;
$showall = null;
...
$canassign = has_capability('local/keyuser:cohortassign', $context);
if (!$manager) {
...
if ($category) {
    ...
    $PAGE->set_url('/local/keyuser/cohort/index.php', array('contextid'=>$context->id));
    $PAGE->set_title($strcohorts);
    $PAGE->set_heading($COURSE->fullname);
    $showall = false;
} else {
    admin_externalpage_setup('keyuser_cohorts', '', null, '', array('pagelayout'=>'report'));
}

$PAGE->requires->js_amd_inline("
require(['jquery'], function(\$) {
    $('body').on('updated', '.keyuser_readonly', function(e) {
        var actioncell = $(this).closest('td').siblings('.action')[0];
        var editactions = $(actioncell).children('.keyuser_action_cohort_edit');
        editactions.remove();
    });
});
");
...
if ($showall) {
    ...
    $cohorts = keyuser_cohort_get_cohorts($context->id, $page, 25, $searchquery);
}
...
$baseurl = new moodle_url('/local/keyuser/cohort/index.php', $params);

if ($editcontrols = keyuser_cohort_edit_controls($context, $baseurl)) {
...
foreach($cohorts['cohorts'] as $cohort) {
    ...
    if ($showall) {
        if ($cohortcontext->contextlevel == CONTEXT_COURSECAT) {
            $line[] = html_writer::link(new moodle_url('/local/keyuser/cohort/index.php' ,
                    array('contextid' => $cohort->contextid)), $cohortcontext->get_context_name(false));
        } else {
            $line[] = $cohortcontext->get_context_name(false);
        }
    }
    $tmpl = new \local_keyuser\core_cohort\output\cohortname($cohort);
    ...
    */
    ...
    $buttons = array();
    if (empty($cohort->component)) {
        $cohortmanager = has_capability('local/keyuser:cohortmanage', $cohortcontext);
        $cohortcanassign = has_capability('local/keyuser:cohortassign', $cohortcontext);

        $urlparams = array('id' => $cohort->id, 'returnurl' => $baseurl->out_as_local_url(false));
        $showhideurl = new moodle_url('/local/keyuser/cohort/edit.php', $urlparams + array('sesskey' => sesskey()));
        if ($cohortmanager && !$cohort->readonly) {
            if ($cohort->visible) {
                $showhideurl->param('hide', 1);
                $visibleimg = $OUTPUT->pix_icon('t/hide', get_string('hide'));
                $buttons[] = html_writer::link($showhideurl, $visibleimg, array('class'=>'keyuser_action_cohort_edit','title' => get_string('hide')));
            } else {
                $showhideurl->param('show', 1);
                $visibleimg = $OUTPUT->pix_icon('t/show', get_string('show'));
                $buttons[] = html_writer::link($showhideurl, $visibleimg, array('class'=>'keyuser_action_cohort_edit','title' => get_string('show')));
            }
            $buttons[] = html_writer::link(new moodle_url('/local/keyuser/cohort/edit.php', $urlparams + array('delete' => 1)),
                $OUTPUT->pix_icon('t/delete', get_string('delete')),
                array('class'=>'keyuser_action_cohort_edit','title' => get_string('delete')));
            $buttons[] = html_writer::link(new moodle_url('/local/keyuser/cohort/edit.php', $urlparams),
                $OUTPUT->pix_icon('t/edit', get_string('edit')),
                array('class'=>'keyuser_action_cohort_edit','title' => get_string('edit')));
            $editcolumnisempty = false;
        }
        if ($cohortcanassign) {
            $buttons[] = html_writer::link(new moodle_url('/local/keyuser/cohort/assign.php', $urlparams),
                $OUTPUT->pix_icon('i/users', get_string('assign', 'core_cohort')),
                array('class'=>'keyuser_action_cohort_assign','title' => get_string('assign', 'core_cohort')));
            $editcolumnisempty = false;
        }
    }
...
*/
$table->head  = array(get_string('name', 'cohort'), get_string('description', 'cohort'),
                      get_string('memberscount', 'cohort'), get_string('component', 'cohort'));
//$table->colclasses = array('leftalign name', 'leftalign id', 'leftalign description', 'leftalign size','centeralign source');
$table->colclasses = array('leftalign name', 'leftalign description', 'leftalign size','centeralign source');
if ($showall) {
