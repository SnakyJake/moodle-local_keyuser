...
if ($id) {
    ...
    $cohort->name        = '';
    $cohort->description = '';
    $cohort->readonly = false;
}

require_capability('local/keyuser:cohortmanage', $context);

if ($returnurl) {
    $returnurl = new moodle_url($returnurl);
} else {
    $returnurl = new moodle_url('/local/keyuser/cohort/index.php', array('contextid'=>$context->id));
}
...
$baseurl = new moodle_url('/local/keyuser/cohort/edit.php', array('contextid' => $context->id, 'id' => $cohort->id));
...
if ($context->contextlevel == CONTEXT_COURSECAT) {
    ...
    navigation_node::override_active_url(new moodle_url('/local/keyuser/cohort/index.php', array('contextid'=>$cohort->contextid)));

} else {
    navigation_node::override_active_url(new moodle_url('/local/keyuser/cohort/index.php', array()));
}

if ($delete and $cohort->id and !$cohort->readonly) {
...
$editform = new keyuser_cohort_edit_form(null, array('editoroptions'=>$editoroptions, 'data'=>$cohort, 'returnurl'=>$returnurl));
...
if ($editform->is_cancelled()) {
    ...
    if ($data = $editform->get_data()) {
        ...
        if ($returnurl->get_param('showall') || $returnurl->get_param('contextid') == $data->contextid) {
            ...
            redirect(new moodle_url('/local/keyuser/cohort/index.php', array('contextid' => $data->contextid)));
        }
    }
}
